...
function ShareModalContent({ post, onClose }) {
  const { user } = useAuth()
  ...
  const shareText = `Check out this post on Threadly\n${postUrl}`
  ...
  async function fetchFollowing() {
    try {
      const res = await axiosInstance.get(`/users/${user._id}/following`)
      const data = res.data
      // This endpoint returns the array directly, not nested in an object
    ...
    try {
      ...
      await Promise.all(
        selected.map(recipientId =>
          axiosInstance.post('/messages', { receiverId: recipientId, text: shareText, postId: post._id })
        )
      )
      setSent(true)
      // Refresh DM conversations so the shared post appears
      refreshConvos()
  ...
  async function handleShareOption(id) {
    if (id === 'whatsapp') {
      ...
      window.open(`mailto:?subject=Check this post&body=${encodeURIComponent(shareText)}`, '_blank')
    } else if (id === 'copy') {
      await navigator.clipboard.writeText(postUrl)
      setCopiedLink(true)
      ...
      if (navigator.share) {
        navigator.share({ title: 'Threadly post', text: shareText, url: postUrl }).catch(() => {})
      }
    }
  }
        ...
        >
          ...
          <div style={{ padding: '6px 22px 0' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 22 }}>
              <span style={{ fontWeight: 700, fontSize: 18, color: '#111827' }}>Share</span>
              <button
                onClick={handleClose}
                style={{
            ...
            {loading ? (
              ...
              <p style={{ fontSize: 13, color: '#9CA3AF', marginBottom: 22, textAlign: 'center' }}>
                Follow people to send posts directly
              </p>
            ) : (
              <>
...
export default function ShareModal({ post, onClose }) {
  return createPortal(
    <ShareModalContent post={post} onClose={onClose} />,
    document.body
  )
}